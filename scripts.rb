...
def install_mesosphere(mode, control_ip, node_ip, attributes)
    $script = <<SCRIPT
bash /vagrant/mesosphere-install.sh #{mode} #{control_ip} #{node_ip} #{attributes}
SCRIPT
    return $script
end